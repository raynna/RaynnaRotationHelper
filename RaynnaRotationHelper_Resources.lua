...
local RESOURCE_TOOLTIPS = {
    COMBO_POINTS = { "Combo Points", "Resource used by feral druids and rogues. Builders generate combo points on your target, and finishers consume them for stronger effects." },
    ARCANE_CHARGES = { "Arcane Charges", "Arcane mage resource. Arcane Blast and some arcane spells generate charges, increasing arcane damage and mana cost until charges are spent or expire." },
    HOLY_POWER = { "Holy Power", "Resource used by all paladins. Some abilities generate holy power and others consume either all available or 3 holy power." },
    CHI = { "Chi", "Monk resource generated by builders and spent on stronger abilities, healing, or defensive effects depending on specialization." },
    SHADOW_ORBS = { "Shadow Orbs", "Shadow priest resource generated by select shadow spells and consumed by Devouring Plague for increased damage." },
    SOUL_SHARDS = { "Soul Shards", "Affliction warlock resource used for powerful soulburn and damage effects. Generated through warlock mechanics and spent by key abilities." },
    BURNING_EMBERS = { "Burning Embers", "Destruction warlock resource built by fire spells and spent on powerful ember abilities such as Chaos Bolt and Ember Tap." },
    DEMONIC_FURY = { "Demonic Fury", "Demonology warlock resource generated by caster-form abilities and spent while transformed with Metamorphosis." },
}
...
local function ResourceTooltipText(resource)
    local data = RESOURCE_TOOLTIPS[resource or ""]
    if data then return data[1], data[2] end
    return "Class Resource", "This display shows your current class resource and available maximum for the active specialization."
end

local function TooltipFrame(region)
    if not region then return nil end
    return region.frame or region.region or region
end

local function AttachResourceTooltip(region)
    local frame = TooltipFrame(region)
    if not frame or frame.RaynnaResourceTooltipAttached then return end
    frame.RaynnaResourceTooltipAttached = true
    if frame.EnableMouse then frame:EnableMouse(true) end
    if frame.SetMouseClickEnabled then frame:SetMouseClickEnabled(false) end
    if frame.SetScript then
        frame:SetScript("OnEnter", function(self)
            local count, maxCount, resource = 0, 0, nil
            if _G.RaynnaRotationHelperGetResourceInfo then
                count, maxCount, resource = _G.RaynnaRotationHelperGetResourceInfo()
            end
            local title, body = ResourceTooltipText(resource)
            if GameTooltip then
                GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
                GameTooltip:SetText(title, 1, 1, 1, 1, true)
                if maxCount and maxCount > 0 then
                    GameTooltip:AddLine(tostring(count or 0) .. " / " .. tostring(maxCount), 0.85, 0.85, 0.85, true)
                end
                GameTooltip:AddLine(body, 0.72, 0.82, 1, true)
                GameTooltip:Show()
            end
        end)
...
function _G.RaynnaRotationHelperUpdateResourceVisuals(elapsed)
    ...
    for i = 1, #RESOURCE_IDS do
        ...
        AttachResourceTooltip(fill)
        AttachResourceTooltip(outline)
        AttachResourceTooltip(border)
